...
<x-modal name="groups-modal" max-width="3xl" :show="$errors->group->any()" focusable>
    <div x-data="{}" x-init="setTimeout(() => { if(window.lucide) window.lucide.createIcons(); }, 150)">
        <div class="flex flex-col max-h-[70vh] w-full overflow-hidden">
            ...
            <div class="flex flex-col lg:flex-row flex-1 min-h-0">
                ...
                <div class="lg:w-[38%] border-b lg:border-b-0 lg:border-r border-white/10 p-6 overflow-y-auto">
                    ...
                    <form
                        method="POST"
                        action="{{ route('mind-groups.store') }}"
                        class="space-y-3"
                    ...
                    >
                        ...
                        <button
                            type="submit"
                            :disabled="submitting"
                            class="w-full py-2.5 bg-white text-black text-sm font-bold hover:scale-[1.02] transition rounded-lg disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 inline-flex items-center justify-center gap-2"
                        >
                            Criar Grupo
                        </button>
                    </form>
                                ...
                                <div x-data="{ editing: {{ $isFailedEdit ? 'true' : 'false' }} }" class="border border-white/10 bg-white/[0.02] rounded-xl overflow-hidden">
                                    ...
                                    <div x-show="!editing" class="px-4 py-3 flex items-center justify-between gap-3">
                                        <div class="flex items-center gap-2.5 min-w-0">
                                            <div class="w-8 h-8 rounded-full border border-white/10 bg-white/[0.04] flex items-center justify-center shrink-0 text-zinc-300">
                                                <span
                                        ...
                                        <div class="flex items-center gap-2 shrink-0">
                                            ...
                                            >
                                                ...
                                                <button
                                                    type="submit"
                                                    :disabled="submitting"
                                                    class="px-2.5 py-1 text-xs text-red-400/70 hover:text-red-400 bg-red-400/[0.05] hover:bg-red-400/[0.10] rounded-lg transition disabled:opacity-50 disabled:cursor-not-allowed"
                                                >
                                                    Excluir
                                                </button>
                                            </form>
                                    ...
                                    >
                                        ...
                                        <div class="flex gap-2 pt-1">
                                            <button
                                                type="submit"
                                                :disabled="submitting"
                                                class="px-4 py-2 bg-white text-black text-xs font-bold hover:scale-[1.02] transition rounded-lg disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 inline-flex items-center gap-2"
                                            >
                                                Salvar
                                            </button>
                                            <button
